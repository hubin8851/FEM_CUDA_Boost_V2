...
public:
	// 默认构造函数. Initializes m_Tag to an invalid value.
	record_tagger_feature();
	// 拷贝构造函数. Initializes m_Tag to a value, equivalent to that.m_Tag.
...
logging::record record_tagger_feature< BaseT >::open_record_unlocked(ArgsT const& args)
{
	...
	if (!tag_value.empty())
	{
		...
		std::pair< logging::attribute_set::iterator, bool > res 
			= BaseT::add_attribute_unlocked("Tag", attrs::constant< std::string >(tag_value));
		if (res.second)
			tag = res.first;
	}
...
inline std::ostream& operator<< (std::ostream& strm, HBXDef::MySeverity_level level)
{
	const char* levels[] =
	{
		"trace",
...
void manual_logging()
{
	my_logger< HBXDef::MySeverity_level > logger;

	logging::record rec = logger.open_record((keywords::severity = HBXDef::MySeverity_level::trace, my_keywords::tag = "GUI"));
	if (rec)
	{
...
void logging_function()
{
	...
	LOG_WITH_TAG(logger, HBXDef::MySeverity_level::trace, "GUI") << "The user has confirmed his choice";
}
//]
...
void Instance_Tag()
{
	...
	sink->locked_backend()->add_stream(
		boost::make_shared< std::ofstream >("TagSample.log"));

	sink->set_formatter
	(
		expr::stream
		<< expr::attr< unsigned int >("LineID")
		<< ": <" << expr::attr< HBXDef::MySeverity_level >("Severity")
